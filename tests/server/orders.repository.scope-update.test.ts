...
describe('OrdersRepository.list owner scope handling', () => {
  it('scopes to userId when ownerIds are undefined', async () => {
    const { client, filters } = makeOrdersClientMock()
    const repo = new OrdersRepository(client as any)
