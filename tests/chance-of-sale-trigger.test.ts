...
import { invokeChanceOfSaleGeneration } from '../src/app/api/sessions/[id]/chance-of-sale/helpers'

describe('invokeChanceOfSaleGeneration', () => {
  it('runs the generator with provided client and ids', async () => {
    const generator = vi.fn().mockResolvedValue({ markdown: '# ok' })
    const supabase = {} as any

    const result = await invokeChanceOfSaleGeneration(supabase, 'user-1', 'session-1', { generator })
