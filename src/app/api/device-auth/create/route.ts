...
export async function POST() {
  try {
    if (!supabaseAdmin) return new Response('Service role not configured', { status: 500 })
    ...
    try { rateLimit(user.id); if (ip) rateLimit(`ip:${ip}`) } catch { return new Response('Too Many Requests', { status: 429 }) }

    // Get current session tokens from cookies
    const { data: sessionData } = await supabase.auth.getSession()
    const tokens = sessionData?.session
    if (!tokens?.access_token || !tokens?.refresh_token) return new Response('No session', { status: 400 })
    ...
    if (upsertError) return new Response('Create failed', { status: 500 })

    return new Response(JSON.stringify({ code, expires_in: TTL_SECONDS }), { status: 200, headers: { 'Content-Type': 'application/json' } })
  } catch (e) {
