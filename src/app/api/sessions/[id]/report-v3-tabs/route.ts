...
const MAX_PENDING_TABS_PER_USER = Number(process.env.REPORT_V3_TABS_MAX_PENDING_PER_USER ?? 5)
...
export async function POST(
	...
	try {
		...
		// Lightweight backpressure: cap queued+running tabbed reports per user
		if (MAX_PENDING_TABS_PER_USER > 0) {
			try {
				const { count, error: countError } = await (supabase as any)
					.from('session_reports_v3_tabs')
					.select('session_id', { count: 'exact', head: true })
					.in('status', ['queued', 'running'])
				if (!countError && typeof count === 'number' && count >= MAX_PENDING_TABS_PER_USER) {
					return NextResponse.json(
						{ error: 'Too many pending tabbed reports for this user. Please wait for existing reports to complete.' },
						{ status: 429 }
					)
				}
			} catch (e) {
				console.error('Report V3 Tabs backpressure check failed (non-fatal):', e)
			}
		}
